...
const count = () => {
  for (let i = 0; i < props.events.length; i++) {
    ...
    const repeatEmail = numOfEmail.value.filter(
      (numOfEmail) => numOfEmail.currentEmail === currentEmail
    )
...
<template>
  ...
  <div>
    <div
      class="grid grid-cols-1 gap-1 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-2 bg-[#c4c4c4] p-4"
    >
      <div
        v-for="event in events"
        :key="event.id"
        class="bg-white m-4 px-4 pt-2 pb-4 leading-8 border-4 rounded border-stone-900 text-sm"
      >
        <ul>
          <li class="flex justify-end font-bold">
            <span class="text-[#878787] mr-1">CATEGORY: </span
          ...
          <li class="font-bold">
            <span class="text-[#878787]">START TIME: </span
            >{{ dayjs(event.eventStartTime).format('DD MMMM YYYY HH.mm') }}
          </li>
          <li class="font-bold">
            <span class="text-[#878787] mr-1">DURATION: </span
            ><span class="mr-28">{{ event.eventDuration }} mins</span>
          </li>
          <li class="relative mt-6">
            <button
              v-show="
                refreshTokenDecode.role !== 'LECTURER' ||
                tokenDecode.role !== 'LECTURER'
              "
            ...
            <span
              ...
              class="grid justify-end leading-3 justify-items-center absolute bottom-0 right-0 hover:cursor-pointer"
            >
              <span
                class="iconify"
                data-icon="noto:bear"
              ...
              ></span>
              <span
                class="text-[#878787] font-bold underline hover:text-black"
